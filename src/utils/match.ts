...
import { TaskMetadata } from '../index';
 ...
 *
 * @param {ITaskContext} ctx
 * @param {TaskMetadata} decor
 * @param {TaskMetadata} match
 * @returns
 */
function matchTaskInfo(ctx: ITaskContext, decor: TaskMetadata, match: TaskMetadata) {
...
function matchTaskGroup(tinfo: ITaskInfo, match: ITaskInfo): boolean {
    if (tinfo.group && match && match.group) {
        if (isString(match.group)) {
            if (isString(tinfo.group) && tinfo.group !== match.group) {
                return false;
            } else if (isArray(tinfo.group) && tinfo.group.indexOf(match.group) < 0) {
                return false;
            }
        } else if (isArray(match.group)) {
            if (isString(tinfo.group) && match.group.indexOf(tinfo.group) < 0) {
                return false;
            } else if (isArray(tinfo.group) && !contains(tinfo.group, match.group)) {
                return false;
            }
        }
    } else if (tinfo.group) {
