        ...
        if (tname) {
            ...
            if ((ctx.oper & Operation.watch) && (infoOper & Operation.autoWatch)) {
                ...
                gulp.task(wname, () => {
                    ...
                    watch(src, null, (cb) => {
                        runSequence(gulp, isArray(tname) ? tname : [<string>tname]);
                    });
                });
...
export function zipSequence(gulp: Gulp, tasks: Src[], ctx: ITaskContext, zipName?: ZipTaskName): string {
    if (tasks.length > 1) {
        ...
        first = isArray(first) ? _.first(first) : first;
        let last = _.last(tasks);
        last = isArray(last) ? _.last(last) : last;
        let name = `${first}-${last}`;
...
export function flattenSequence(gulp: Gulp, tasks: Src[], ctx: ITaskContext, zipName?: ZipTaskName): string[] {
    ...
    _.each(tasks, tk => {
        if (isArray(tk)) {
            ...
            let zipSrc: Src[] = (_.some(tk, t => isArray(t))) ? tk : [tk];
...
function startTask(gulp: Gulp, task: Src): Promise<any> {
    ...
    return new Promise((reslove, reject) => {
        let tskmap: any = {};
        _.each(isArray(task) ? task : [task], t => {
            tskmap[t] = false;
        });
        taskErr = (err) => {
            reject(err);
